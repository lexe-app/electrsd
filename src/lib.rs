...
pub struct Conf<'a> {
    ...
    ///
    /// Defaults are tuned to minimize resource use on small regtest chains.
    /// Replace this vector to opt out of those defaults. Note that `db-dir`,
    /// `cookie`, `daemon-rpc-addr`, `jsonrpc-import`, `electrum-rpc-addr`,
    /// `monitoring-addr`, and `http-addr` cannot be used because they are
...
impl Default for Conf<'_> {
    fn default() -> Self {
        Conf {
            args: vec![
                "--lightmode",
                "--daemon-parallelism=1",
                "--initial-sync-batch-size=1",
                "--db-parallelism=1",
                "--db-block-cache-mb=1",
